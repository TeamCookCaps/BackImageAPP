const ImageInfo = {
  ...
  getSearchResult: (
    word
  ) => `select id,uid,image_url,image_date,image_location,image_width,image_height,b.category_name
  ...
  getSearchWordColorResult: (
    word,
    image_id
  ) => `select id,uid,image_url,image_date,image_location,image_width,image_height,b.category_name,rgb_info
                                                from ImageInfo a INNER JOIN ImageCategory b ON a.id = b.image_id 
                                                INNER JOIN parent_category c on b.category_name = c.category_name 
                                                INNER JOIN (
  ...
  getSimilarColors: (minR, maxR, minG, maxG, minB, maxB) =>
    `SELECT image_id FROM palette WHERE r BETWEEN ${minR} AND ${maxR} AND g BETWEEN ${minG} AND ${maxG} AND b BETWEEN ${minB} AND ${maxB}`,
};
